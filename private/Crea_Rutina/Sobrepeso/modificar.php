...
<div id="boxContent"> 
  ...
  <form method="POST" action="modificar_pr.php?id=<?php echo $row['product_id'];?>" enctype="multipart/form-data">
	<div id="formContent">
	<div class="contenedorForm">
		<label for="nombre">Nombre R:</label>
		<input class="campoTexto" type="text" required id="nombre" name="nombre" value="<?php echo $row['product_title'];?>">
  </div>
...
<select name="idCategoria" id="" required class="campoTexto" style="height: auto;">
	...
	<div class="contenedorForm">
		...
		<textarea class="campoTexto" id="mensaje" name="ingredientes"  required><?php echo $row['product_desc'];?></textarea>
  </div>
  <div class="contenedorForm">
		<label for="mensaje">Preparacion:</label>
		<textarea class="campoTexto" name="preparacion" id="mensaje" required><?php echo $row['product_keywords'];?></textarea>
  </div>
  <div class="contenedorForm">
		<label for="mensaje">T.Calorias:</label>
		<input class="text" name="calorias" required value="<?php echo $row['product_price'];?>"></input>
  </div>
  <div class="contenedorForm">
  ...
  <form method="POST" action="modificar_pr.php?id=<?php echo $row['product_id'];?>" enctype="multipart/form-data">
		...
		<label for="radioNombre">Nombre:</label>
			<input type="text" id="nombreMobile" class="campoTexto" name="nombre" required placeholder="Nombre de la reseta" value="<?php echo $row['product_title'];?>">
    ...
    while($tiempos = mysqli_fetch_row($v)){
      ...
      <textarea id="mensajeMobile" name="ingredientes" class="campoTexto" required placeholder="Ingredientes"><?php echo $row['product_desc'];?></textarea>
      
      <input type="radio" name="radio-3" class="radio" id="radioPreparacion">
		<label for="radioPreparacion">Preparacion:</label>
			<textarea id="preparacionMobile" name="preparacion" class="campoTexto" required placeholder="Preparacion"><?php echo $row['product_keywords'];?></textarea>


       
      <input type="radio" name="radio-3" class="radio" id="radioCalorias">
		<label for="radioCalorias">Preparacion:</label>
			<textarea id="caloriasMobile" name="calorias" class="campoTexto" required placeholder="Calorias"><?php echo $row['product_price'];?></textarea>
