 ...
 */
  ...
  */
	...
	if(isset($_POST['page']))
	{
		$sql="SELECT * FROM products where peso = 1";
 ...
 */
	...
	if(isset($_POST['getProduct'])){
		...
		if(isset($_POST['price_sorted'])){
			$product_query="SELECT * FROM products where peso = 1 ORDER BY product_price";
		}
		elseif(isset($_POST['pop_sorted'])){
			$product_query="SELECT * FROM products where peso = 1 ORDER BY RAND()";
		}
		else{
		$product_query="SELECT * FROM products where product_cat = 1 and peso = 1 LIMIT $start,$limit";
		}
...
/** ordena los productos segun la categoria selecionada */
	if(isset($_POST['get_selected_Category']) || isset($_POST['get_selected_brand']) || isset($_POST['search']) || isset($_POST['price_sorted']))
	{
		if(isset($_POST['get_selected_Category'])){
			$cid=$_POST['cat_id'];
			$sql="SELECT * FROM products WHERE product_cat=$cid and peso = 1";
		}
		elseif(isset($_POST['get_selected_brand'])){
			$bid=$_POST['brand_id'];
			$sql="SELECT * FROM products WHERE product_brand=$bid";
			if(isset($_POST['price_sorted'])){
			$sql="SELECT * FROM products where peso = 1 ORDER BY product_price";
			}
		}
...
/**con sulta la rutina ya creada con el id del tiempo de comida y el id del usuario */
				$sqli = "SELECT * FROM cart WHERE product_cat = '$cat' AND user_id = '$uid' AND peso = 1";
				$run_queryi=mysqli_query($conn,$sqli);
				$counti=mysqli_num_rows($run_queryi);

				if($cat == '1'){
                    if($counti>2){/**si ya hay 2 de el mismo tiempo registrado */
						echo "
						<script>
				...
				$peso = $row["peso"];
				$sql="INSERT INTO cart(p_id,ip_add,user_id,product_title,product_image,qty,price,total_amount,product_cat,peso) VALUES('$pid','0.0.0.0','$uid','$pro_title','$pro_image','1','$pro_price','$pro_price','$idCategoria','1')";
				$run_query = mysqli_query($conn,$sql);
				if($run_query){
					echo "
					<script>
	...
	{
		$uid=$_SESSION['uid'];
		$sql="SELECT * FROM cart WHERE user_id='$uid' AND peso = 1 ORDER BY product_cat";
		$run_query=mysqli_query($conn,$sql);
		$count=mysqli_num_rows($run_query);
		if($count>0){
			$i=1;
	...
	if(isset($_POST['cartcount'])){
		...
		$uid=$_SESSION['uid'];
		$sql="SELECT * FROM cart WHERE user_id='$uid' AND peso = 1";
		$run_query=mysqli_query($conn,$sql);
		$count=mysqli_num_rows($run_query);
		echo $count;
		}
